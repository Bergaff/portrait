...
function updateAuthUI(user) {
    ...
    // Иконка уже в HTML, ничего дополнительно выводить не нужно
}
...
function saveUserStats() {
    if (currentUser) localStorage.setItem("userStats_" + currentUser.id, JSON.stringify(userStats));
}
...
function openProfile() {
    ...
    if (isPro) {
        ...
        html += '<p style="color:#888;font-size:12px;margin:8px 0;">Базовый доступ</p>';
...
function upgradeToPro() { alert("Скоро! Напишите на support@portrait.app"); }
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                try {
                    ...
                    showCityConfirm("Ваш город", pos.coords.latitude, pos.coords.longitude);
                }
            },
            () => detectByIP(),
            { timeout: 8000 }
...
const CAT_MAP = {
    ...
    "Шопинг":["clothes","supermarket","convenience","electronics"],
    "Спорт":["gym","fitness_centre","sports_centre"],
    "Образование":["school","kindergarten","university","library"],
    "Досуг":["cinema","theatre","museum","playground"],
    "Разнообразие":[]
};
const CAT_COLORS = {
...
function showScores(s) {
    ...
    if (state.activeFilter) {
        html += '<div style="text-align:center;font-size:11px;color:#667eea;margin-bottom:6px;">Фильтр: '+state.activeFilter+'</div>';
    }
    metrics.forEach(m => {
        ...
        html += '<span style="width:8px;height:8px;border-radius:50%;background:'+(CAT_COLORS[m.label]||"#667eea")+';margin-right:6px;display:inline-block;flex-shrink:0;"></span>';
...
async function generateReport() {
    ...
    try {
        ...
        if (!r.ok) { addBotMessage("❌ Ошибка сервера " + r.status); return; }
        const data = await r.json();
...
function renderCharts() {
    ...
    state.barChart = new Chart(document.getElementById("barChart").getContext("2d"), {
        type:"bar",
        data:{labels:barLabels,datasets:[{data:barValues,backgroundColor:colors,borderRadius:4}]},
        options:{indexAxis:"y",responsive:true,maintainAspectRatio:false,
            scales:{x:{max:100,ticks:{color:"#888"}},y:{ticks:{color:"#ccc"}}},
    ...
    state.radarChart = new Chart(document.getElementById("radarChart").getContext("2d"), {
        ...
        data:{labels:barLabels,datasets:[{
            label:"Район",data:barValues,
            backgroundColor:"rgba(102,126,234,0.2)",
            borderColor:"#667eea",borderWidth:2,
            pointBackgroundColor:"#667eea",pointRadius:3
        }]},
        options:{responsive:true,maintainAspectRatio:false,
            scales:{r:{min:0,max:100,ticks:{color:"#666",backdropColor:"transparent"}}},
            plugins:{legend:{display:false}}
        }
    });
}
...
async function exportPDF() {
    ...
    try {
        ...
        for (let i=0;i<pages;i++) {
            ...
            pc.width=canvas.width; pc.height=Math.min(ph,canvas.height-i*ph);
...
function setChatBusy(b) {
    state.chatBusy=b;
    document.querySelectorAll(".btn-quick").forEach(x=>{x.disabled=b;x.style.opacity=b?"0.4":"1";});
...
async function sendMessage() {
    ...
    try {
        ...
        const d=await r.json(); removeLoading(); addBotMessage(d.answer);
